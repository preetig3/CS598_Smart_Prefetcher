...
def save_metrics_log(metrics):
    """save metrics to baseline_S3_dataloader_metrics.log"""
    try:
        with open('baseline_S3_dataloader_metrics.log', 'w') as f:
            f.write("# Baseline S3 DataLoader Metrics\n")
...
def train_model(dataloader, num_epochs: int = 5, learning_rate: float = 0.001):
    """
    Train the baseline model with PyTorch's built-in prefetching
    """
    print("starting baseline training (PyTorch num_workers)")
    print("-----")
    
    # create model and optimizer (this is the optimal model for cifar)
    model = nn.Sequential(
    ...
    print("\n")
    print("----")
    print("Baseline training is complete")
    print("----")
...
if __name__ == "__main__":
    ...
    print("Baseline: pyTorch num_workers prefetching")
    ...
    print(f"\nMetrics saved to baseline_S3_dataloader_metrics.log")
    
    print(f"\nthis baseline uses PyTorch's num_workers={NUM_WORKERS} for prefetching")
