...
def clear_pipeline_log():
    try:
        ...
        with open(logger, 'w') as f:
            f.write("# Pipeline Activity Log - ResNet152 with RAM+SSD+RPC Caching\n")
            f.write("# FETCH: timestamp,worker_id,image_idx,fetch_time,source (RAM|SSD|RPC|S3)\n")
            f.write("# PROCESS: timestamp,main_id,batch_size,indices\n")
    except Exception:
        pass

def get_epoch_stats_from_log(epoch_start_time, epoch_end_time):
    stats = {'ram_hits': 0, 'ssd_hits': 0, 'rpc_hits': 0, 's3_fetches': 0}
    try:
...
def save_metrics_log(metrics, cache_stats):
    try:
        total_ops = (cache_stats.get('ram_hits', 0) +
                     ...
                     cache_stats.get('rpc_hits', 0) +
                     cache_stats.get('s3_fetches', 0))
        if total_ops > 0:
            ram_pct = cache_stats['ram_hits'] / total_ops * 100
            ssd_pct = cache_stats['ssd_hits'] / total_ops * 100
            rpc_pct = cache_stats['rpc_hits'] / total_ops * 100
            s3_pct = cache_stats['s3_fetches'] / total_ops * 100
            cache_hit_rate = (cache_stats['ram_hits'] + cache_stats['ssd_hits'] + cache_stats['rpc_hits']) / total_ops * 100
        else:
            ram_pct = ssd_pct = rpc_pct = s3_pct = cache_hit_rate = 0.0

        with open('cached_S3_dataloader_resnet152_metrics.log', 'w') as f:
            f.write("# Cached S3 DataLoader Metrics - ResNet152 (RAM + SSD + RPC-over-Redis owner map)\n")
            ...
            f.write(f"RPC Owner Hits:    {cache_stats.get('rpc_hits', 0)} ({rpc_pct:.1f}%)\n")
            ...
            f.write("# Analysis:\n")
            f.write(f"L1+L2+RPC eliminated ~{cache_hit_rate:.1f}% of S3 calls.\n")
    except Exception as e:
...
class OwnerRegistry:
    ...
    Tracks which worker owns an index via Redis string keys:
      ...
      value: "host:port"
      TTL:   owner_ttl_sec (auto-cleans stale owners)
    """
    def __init__(self, host, port, db, password, prefix, owner_ttl_sec=600):
        self.r = redis.Redis(host=host, port=port, db=db, password=password, decode_responses=True, socket_timeout=3)
        self.prefix = prefix if prefix.endswith(":") else (prefix + ":")
        self.owner_ttl = int(owner_ttl_sec)

    def owner_key(self, idx: int) -> str:
        return f"{self.prefix}owner:{idx}"

    def set_owner(self, idx: int, endpoint: str):
        try:
            self.r.set(self.owner_key(idx), endpoint, ex=self.owner_ttl)
        except Exception:
            pass

    def get_owner(self, idx: int):
        try:
            return self.r.get(self.owner_key(idx))
        except Exception:
            return None

    def clear_owner(self, idx: int):
        try:
            self.r.delete(self.owner_key(idx))
        except Exception:
            pass
...
class WorkerRPC:
    """
    Serves fetch(idx) -> xmlrpc.client.Binary or None
    Returns a pickled payload (uint8 CHW ndarray, label) wrapped as Binary.
    The worker uses its own cache (RAM->SSD) to answer.
    """
    def __init__(self, get_from_cache_callable):
        self._get_local = get_from_cache_callable

    def fetch(self, idx):
        # Try local RAM/SSD only (no RPC/S3 recursion here!)
        data = self._get_local(idx, allow_rpc=False)
        if data is None:
            return None
        img_t, label = data
        arr = (img_t.clamp(0, 1) * 255.0).to(torch.uint8).cpu().numpy()
        payload = pickle.dumps((arr, int(label)), protocol=pickle.HIGHEST_PROTOCOL)
        return xmlrpc.client.Binary(payload)

#  L1 (RAM) + L2 (SSD/worker) + L3 (RPC via owner map) Cache 
class RamSsdRpcCache:
    """
    Per-worker L1 RAM cache + per-worker L2 SSD cache + shared owner map (Redis) + RPC fetch.
    Lookup: RAM -> SSD -> RPC(owner) -> (caller falls back to S3)
    On S3 fetch: put() writes to SSD and RAM, and registers self as owner in Redis.
    """
    def __init__(
        self,
        ...
        rpc_port: int = 0,                    # 0 = auto-choose free port
        rpc_timeout_sec: float = 3.0,
        owner_announce_key: str = None        # optional: a heartbeat key for this worker if desired
    ):
        # L1 (RAM)
        ...
        # Owner registry + RPC
        self.registry = owner_registry
        self.rpc_host = rpc_bind_host
        ...
        self._local_stats = {'ram_hits': 0, 'ssd_hits': 0, 'rpc_hits': 0, 's3_fetches': 0, 'ram_evictions': 0}
    ...
    def _ensure_rpc_server(self):
        if self._rpc_server is not None:
            return
        # Bind to a free port if rpc_port == 0
        port = self.rpc_port
        if port == 0:
        ...
        self.rpc_port = port
        self.rpc_endpoint = f"http://{self.rpc_host}:{self.rpc_port}"

        # Server object
        class _Req(SimpleXMLRPCRequestHandler):
            rpc_paths = ('/RPC2',)

        # The fetch handler uses _get_local_data (no RPC recursion, no S3)
        def _get_local_data(idx, allow_rpc=False):
            # Only RAM/SSD here (server side should never call out)
        ...
        self._rpc_server = ThreadingXMLRPCServer((self.rpc_host, self.rpc_port), requestHandler=_Req, logRequests=False, allow_none=True)
        self._rpc_server.register_introspection_functions()
        self._rpc_server.register_instance(WorkerRPC(_get_local_data))
    ...
    def get(self, idx: int):
        ...
        # L3 RPC via owner map
        owner = self.registry.get_owner(idx)
        if owner and (owner != f"{self.rpc_host}:{self.rpc_port}"):
            try:
                # owner format "host:port"
                srv = xmlrpc.client.ServerProxy(f"http://{owner}", allow_none=True)
                # default socket timeout
                old_timeout = socket.getdefaulttimeout()
                socket.setdefaulttimeout(self.rpc_timeout_sec)
                res = srv.fetch(idx)
                socket.setdefaulttimeout(old_timeout)
                if isinstance(res, xmlrpc.client.Binary):
                    arr, label = pickle.loads(res.data)  # arr is uint8 CHW
                    img_t = torch.from_numpy(arr).to(torch.float32) / 255.0
                    data = (img_t, int(label))
                    # Promote into our caches
                    self._put_ssd(key, data)
                    self._put_ram(key, data)
                    self._local_stats['rpc_hits'] += 1
                    self._update_global_stats('rpc_hits')
                    return data, 'RPC'
                else:
                    # Stale owner mapping — clear it
                    self.registry.clear_owner(idx)
            except Exception:
                # RPC failed — clear stale owner and continue to MISS
                self.registry.clear_owner(idx)
    ...
    def put(self, idx: int, image_tensor: torch.Tensor, label: int):
        ...
        # Register self as owner for this idx
        try:
...
class CachedS3CIFAR10Dataset(Dataset):
    ...
    Lookup: RAM -> SSD -> RPC(owner) -> S3
    On S3: writes into SSD and RAM, then registers ownership in Redis.
    """
    def __init__(
        self,
        bucket_name: str,
        ...
        redis_key_prefix: str = "cifar10",
        owner_ttl_sec: int = 600,
        # RPC
        rpc_bind_host: str = '127.0.0.1',
        rpc_port: int = 0,
        rpc_timeout_sec: float = 3.0,
        ...
        self.rpc_bind_host = rpc_bind_host
        ...
        print(f"Initialized RPC-backed CACHED S3 dataset with {num_images} images")
        print("Lookup order: RAM (per-worker) -> SSD (per-worker) -> RPC(owner via Redis) -> S3")
    ...
    def _get_cache(self):
        if (self._cache is None) or (self._worker_pid != os.getpid()):
            ...
            registry = OwnerRegistry(
                ...
                password=self.redis_password, prefix=self.redis_key_prefix, owner_ttl_sec=self.owner_ttl_sec
            )
            self._cache = RamSsdRpcCache(
                ...
                rpc_bind_host=self.rpc_bind_host,
    ...
    def __getitem__(self, idx: int):
        ...
        try:
            ...
            # store to L2 + L1 and register owner
...
def train_model(dataloader, cache_stats_file, num_epochs: int = 5, learning_rate: float = 0.001):
    ...
    def read_stats_file():
        stats = {'ram_hits': 0, 'ssd_hits': 0, 'rpc_hits': 0, 's3_fetches': 0, 'ram_evictions': 0}
        try:
            ...
            pass
        total = stats['ram_hits'] + stats['ssd_hits'] + stats['rpc_hits'] + stats['s3_fetches']
        if total > 0:
            stats['ram_hit_rate'] = stats['ram_hits'] / total * 100
            stats['ssd_hit_rate'] = stats['ssd_hits'] / total * 100
            stats['rpc_hit_rate'] = stats['rpc_hits'] / total * 100
            stats['cache_hit_rate'] = (stats['ram_hits'] + stats['ssd_hits'] + stats['rpc_hits']) / total * 100
            stats['s3_fetch_rate'] = stats['s3_fetches'] / total * 100
        else:
            stats['ram_hit_rate'] = stats['ssd_hit_rate'] = stats['rpc_hit_rate'] = stats['cache_hit_rate'] = stats['s3_fetch_rate'] = 0.0
        return stats
    ...
    for epoch in range(num_epochs):
            ...
            try:
                ...
                if batch_count % 10 == 0:
                    ...
                    print(f"Batch {batch_count:3d}/{expected_batches:3d}: "
                          ...
                          f"(RAM {cs['ram_hit_rate']:.1f}%, SSD {cs['ssd_hit_rate']:.1f}%, RPC {cs['rpc_hit_rate']:.1f}%)")
        ...
        print(f"    RPC Hits:    {e_stats['rpc_hits']}")
        print(f"    SSD Hits:    {e_stats['ssd_hits']}")
        print(f"    RAM Hits:    {e_stats['ram_hits']}")
        etotal = sum(e_stats.values())
        if etotal > 0:
            e_hit = (e_stats['ram_hits'] + e_stats['ssd_hits'] + e_stats['rpc_hits']) / etotal * 100
    ...
    total_ops = (final_stats['ram_hits'] + final_stats['ssd_hits'] +
                 final_stats['rpc_hits'] + final_stats['s3_fetches'])
    overall_hit = ((final_stats['ram_hits'] + final_stats['ssd_hits'] + final_stats['rpc_hits']) / total_ops * 100) if total_ops > 0 else 0.0

    print("\n----")
    print("Cached (RAM + SSD + RPC) ResNet152 training is complete")
    print("----")
    print("\nCache Performance (from log):")
    if total_ops > 0:
        ...
        print(f"  RPC Hits:   {final_stats['rpc_hits']} ({final_stats['rpc_hits']/total_ops*100:.1f}%)")
...
def cleanup_redis_keys(host, port, db, password, prefix, mode="scan"):
    ...
    try:
        if mode.lower() == "flushdb":
            ...
            return
        # Targeted cleanup via SCAN on '<prefix>:owner:*'
        cursor = 0
        removed = 0
        pattern = (prefix if prefix.endswith(":") else prefix + ":") + "owner:*"
...
if __name__ == "__main__":
    ...
    # Redis owner map settings
    REDIS_HOST = os.getenv('REDIS_HOST', '127.0.0.1')
    REDIS_PORT = int(os.getenv('REDIS_PORT', 6379))
    REDIS_DB = int(os.getenv('REDIS_DB', 0))
    REDIS_PASSWORD = os.getenv('REDIS_PASSWORD') or None
    REDIS_KEY_PREFIX = os.getenv('REDIS_OWNER_PREFIX', 'cifar10')  # owner map prefix (no trailing : needed)
    OWNER_TTL_SEC = int(os.getenv('OWNER_TTL_SEC', 600))           # seconds

    # RPC settings
    RPC_BIND_HOST = os.getenv('RPC_BIND_HOST', '127.0.0.1')
    RPC_PORT = int(os.getenv('RPC_PORT', 0))                        # 0 = auto
    RPC_TIMEOUT_SEC = float(os.getenv('RPC_TIMEOUT_SEC', 3.0))

    # Redis cleanup mode at end
    REDIS_CLEANUP_MODE = os.getenv("REDIS_CLEANUP_MODE", "scan")    # 'scan' (safe) or 'flushdb' (dedicated DB only)

    print("CACHED: RAM (per-worker) + SSD (per-worker) + RPC(owner via Redis) with ResNet152")
    print("---- Configuration ----")
    ...
    print("  Redis Owner Map:")
    print(f"    host={REDIS_HOST} port={REDIS_PORT} db={REDIS_DB} ttl={OWNER_TTL_SEC}s prefix='{REDIS_KEY_PREFIX}'")
    print("  RPC:")
    print(f"    bind_host={RPC_BIND_HOST} port={RPC_PORT or 'auto'} timeout={RPC_TIMEOUT_SEC}s")
    print("-----------------------")
    ...
    # Dataset (per-worker cache + owner map + RPC created lazily per worker)
    ...
    print(f"DataLoader created with {NUM_WORKERS} workers and RAM+SSD+RPC(owner) caching enabled")
    ...
    total_ops = (cache_stats.get('ram_hits', 0) +
                 ...
                 cache_stats.get('s3_fetches', 0))
    if total_ops > 0:
        hit_rate = (cache_stats['ram_hits'] + cache_stats['ssd_hits'] + cache_stats['rpc_hits']) / total_ops * 100
